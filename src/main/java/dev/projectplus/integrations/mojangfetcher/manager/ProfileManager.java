...
public class ProfileManager {
    ...
    private boolean containsUser(UUID uuid) {
        return profiles.values()
                .stream()
                .anyMatch(profile -> profile.getPlayerUUID().equals(uuid));
    }
    ...
    private Profile pullProfile(UUID uuid) {
        return profiles.values()
                .stream()
                .filter(profile -> profile.getPlayerUUID().equals(uuid))
                .findAny().orElseThrow();
    }
    ...
    public Optional<Profile> getProfile(String name) {
        if (containsKey(name)) {
            ...
            long duration = Instant.now().getNano() - profile.get().getPulledOut().getNano();
